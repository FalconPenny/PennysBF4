...
namespace MKO_MH4ck_v1_1
{
    ...
    {
        public const string AppName = "Penny's Cheat";
        public const string AppTitle = @"//V\\ ||< |[]| # Pernille #";
        ...
        // Not needed - private Thread aimbotStream = null;
        ...
        private void OverlayLoad(object sender, EventArgs e)
        {
            ...
            targetEnemies = new List<GPlayer>();
        ...
        private void KeyDownEvent(int Id, string Name)
        {
            switch ((Keys)Id)
            {
                ...
                case Keys.Delete:
                    ...
                    break;
                case Keys.Right:
                    Increase(currMnIndex);
                    break;
                case Keys.Left:
                    Decrease(currMnIndex);
                    break;
                case Keys.Up:
                    CycleMenuUp();
                    break;
        ...
        private bool bAimFov = false;
        private int iAimFov = 20;
        ...
        private void Decrease(MnIndex index)
        {
            switch (index)
            {
                case MnIndex.MN_AIM_IFOV:
        ...
        private string GetMenuString(MnIndex idx)
        {
            ...
            switch (idx)
            {
                ...
                case MnIndex.MN_AIM_KEY:
                    ...
                    break;
                case MnIndex.MN_AIM_BFOV:
                    result = "AIM BFOV [ " + (bAimFov ? "ON" : "OFF") + " ]";
                    break;
                case MnIndex.MN_AIM_IFOV:
                    result = "AIM IFOV [ " + iAimFov + " ]";
                    break;
                case MnIndex.MN_NO_RECOIL:
        ...
        private void AimUpdateKeys()
        {
            ...
            if ((bAimKeyDefault && IsKeyDown(Manager.VK_MIDBUTTON)) || (!bAimKeyDefault && IsKeyDown(Manager.VK_RIGHTBUTTON)))
            {
                if (localPlayer.CurrentWeapon == null || !localPlayer.CurrentWeapon.IsValid())
                    return;

                /*if (localPlayer.InVehicle)
                    return; - Want vehicle aimbot. */

                if (!(targetEnemies.Count > 0))
                    return;

                GPlayer ClosestEnemy = GetClosestEnemy(targetEnemies, currMnAimMode);
                if (ClosestEnemy.IsValidAimbotTarget(bAimTwoSecondsRule, localPlayer.LastEnemyNameAimed, localPlayer.LastTimeEnemyAimed, bAimAtVehicles))
                {
                    ...
                    if (viewAngle != null)
                    {
                        ...
                        localPlayer.LastTimeEnemyAimed = DateTime.Now;
                    }
                }
            }
        }
        ...
        #region Aimbot: Get Closest Enemy
        private GPlayer GetClosestEnemy(List<GPlayer> _Players, MnAimClosestTo _AimMode)
        {
            IEnumerable<GPlayer> withinFov = _Players.Where(p => p.IsValid() && (!bAimFov || (iAimFov <= p.DistanceToCrosshair)));
            if (_AimMode == MnAimClosestTo.PLAYER)
                return withinFov.OrderBy(p => p.Distance).ThenBy(p => p.DistanceToCrosshair).ThenBy(p => p.Health).First();
            else if (_AimMode == MnAimClosestTo.CROSSHAIR)
                return withinFov.OrderBy(p => p.DistanceToCrosshair).ThenBy(p => p.Distance).ThenBy(p => p.Health).First();
        ...
        private VAngle AimLockAtEnemy(GPlayer _Enemy, MnAimTarget _LockAt)
        {
            ...
            if (_Enemy.Distance > 100.0f)
                correction += localPlayer.CurrentWeapon.BulletInitialPosition.Y;
            ...
            if (LockAt.X != 0f && LockAt.Y != 0f && LockAt.Z != 0f)
            {
                ...
                shootSpace.Y = Origin.Y - localPlayer.MatrixInverse.M42 + correction;
                shootSpace.Z = Origin.Z - localPlayer.MatrixInverse.M43;
                shootSpace = NormalizesVector(shootSpace);

                VAngle GameAngle = new VAngle()
                {
                    Yaw = (float)((-Math.Atan2((double)shootSpace.X, (double)shootSpace.Z)) - localPlayer.Sway.X),
                    Pitch = (float)(Math.Atan2((double)shootSpace.Y, Math.Sqrt((double)(shootSpace.X * shootSpace.X + shootSpace.Z * shootSpace.Z))) - localPlayer.Sway.Y)
                };
                ...
                if (_Enemy.Distance > 100.0f)
                {
                    VAngle Y = new VAngle()
                    {
                        Pitch = (float)Math.Atan2(localPlayer.CurrentWeapon.BulletInitialSpeed.Y, localPlayer.CurrentWeapon.BulletInitialSpeed.Z)
                    };
        ...
        public static Dictionary<int, System.Drawing.PointF> createPoints(System.Drawing.PointF pointUpper, System.Drawing.PointF pointLower, float Distance, bool IsSprinting)
        {
            ...
            List<float>
                pointX = new List<float>(),
                pointY = new List<float>();

            if (pointUpper.X == pointLower.X && pointUpper.Y == pointLower.Y)
            {
                pointX.Add(pointLower.X);
                pointY.Add(pointLower.Y);
                return results;
            }

            bool positiveXmod = true;

            if (pointUpper.X > 0 && pointLower.X < 0)
            {
                positiveXmod = false;
                pointLower.X = maxX - Math.Abs(pointLower.X);
            }
            else if (pointLower.X > 0 && pointUpper.X < 0)
            {
                pointUpper.X = maxX - Math.Abs(pointUpper.X);
                positiveXmod = true;
            }
            else if (pointUpper.X > 0 && pointLower.X > 0)
            {
                if (pointUpper.X > pointLower.X)
                    positiveXmod = false;
                else
                    positiveXmod = true;

                if (maxX - pointUpper.X <= 0.75f && pointLower.X <= 0.75f)
                    pointUpper.X = 0.00f;
            }
            else if (pointUpper.X < 0 && pointLower.X < 0)
            {
            ...
            //else { WriteOnLogFile(String.Format("unhandled --> pointUpper.X: {0}, pointLower.X: {1}", pointUpper.X, pointLower.X)); }

            float pointDifferenceX = 0.00f;

            if (pointUpper.X > pointLower.X)
                pointDifferenceX = pointUpper.X - pointLower.X;
            else
                pointDifferenceX = pointLower.X - pointUpper.X;
            ...
            if (pointUpper.X < pointLower.X)
            {
                if (pointLower.X - pointUpper.X < minDifference)
                    pointX.Add(pointLower.X);
                else
                {
                    int points = 0;
                    while (pointUpper.X < pointLower.X)
                    {
                        pointUpper.X += modX;
                        pointX.Add(pointUpper.X);
                        points++;

                        if (points > maxPoints)
                            break;
                    }
                    pointX[pointX.Count - 1] = pointLower.X;
                }
            }
            else
            {
                if (pointUpper.X - pointLower.X < minDifference)
                    pointX.Add(pointLower.X);
                else
                {
                    if (positiveXmod)
            ...
            // Y-axis
            if (pointUpper.Y < pointLower.Y)
            {
                if (pointLower.Y - pointUpper.Y < minDifference)
                    pointY.Add(pointLower.Y);
                else
                {
                    modY = (pointUpper.Y / dividerY);
                    if (modY < minModY)
                        modY = minModY;

                    int points = 0;
                    while (pointUpper.Y < pointLower.Y)
                    {
                        pointUpper.Y += modY;
                        pointY.Add(pointUpper.Y);
                        points++;

                        if (points > maxPoints)
                            break;
                    }
                    pointY[pointY.Count - 1] = pointLower.Y;
                }
            }
            else
            {
                if (pointUpper.Y - pointLower.Y < minDifference || pointUpper.Y < minDifference)
                    pointY.Add(pointLower.Y);
                else
                {
                    modY = -(pointUpper.Y / dividerY);
            ...
            while (i < pointX.Count || i < pointY.Count)
            {
                results.Add(i, new System.Drawing.PointF(pointX[j], pointY[k])); i++;
